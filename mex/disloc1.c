...
#define PI2INV 0.15915494309190
...
{
	if (pModel[0]<0 || pModel[1]<0 || pModel[2]<0 || pModel[2]<sin(pModel[3]*DEG2RAD)*pModel[1])
		return 0;
	else
		return 1;
}
...
void mexFunction (int nlhs, mxArray *plhs[], int nrhs, const mxArray *prhs[])
{
	...
	if (nrhs < 3 ||  nrhs > 4 || nlhs > 1)
		mexErrMsgTxt("Usage: [G]=Disloc(model,stations,nu,{refstat})");
