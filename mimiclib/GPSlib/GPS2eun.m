function [GPS]=GPStmsr2GPSrate(opt)
...
%
%             - subset     : Find constant offset between common time span of
%                        GPS and InSAR (default)
%
%             - seasonal : Uses a model of linear rate + annual and semi-annual
%                           cycles and solve for InSAR planes that would
%                           minimize the difference with the GPS based model
%			
%             - linear : Uses a model of linear rate and solve for InSAR planes that would
%                         minimize the difference with the GPS based model
%
%
...
if strcmp(method,'linear')==1
	...
	for ni=1:length(GPS)
		...
		if isfield(GPS(ni),'x')
			tmp=pinv([GPS(ni).dates_years ones(size(GPS(ni).dates_years,1),1)])*GPS(ni).x;
			GPS(ni).x_rate=tmp(1);
		end
		if isfield(GPS(ni),'y')
			tmp=pinv([GPS(ni).dates_years ones(size(GPS(ni).dates_years,1),1)])*GPS(ni).y;
                        GPS(ni).y_rate=tmp(1);
                end
%                if isfield(GPS(ni),'lon')
%                        tmp=pinv([GPS(ni).dates_years ones(size(GPS(ni).dates_years,1),1)])*GPS(ni).lon;
%                        GPS(ni).lon_rate=tmp(1);
%                end
%                if isfield(GPS(ni),'lat')
%                        tmp=pinv([GPS(ni).dates_years ones(size(GPS(ni).dates_years,1),1)])*GPS(ni).lat;
%                        GPS(ni).lat_rate=tmp(1);
%               end
                if isfield(GPS(ni),'height')
                        tmp=pinv([GPS(ni).dates_years ones(size(GPS(ni).dates_years,1),1)])*GPS(ni).height;
                        GPS(ni).height_rate=tmp(1);
                end
                if isfield(GPS(ni),'los')
                        tmp=pinv([GPS(ni).dates_years ones(size(GPS(ni).dates_years,1),1)])*GPS(ni).los;
                        GPS(ni).los_rate=tmp(1);
                end

	end

elseif strcmp(method,'GPSseasonal')==1
    ...
    for ni=1:length(GPS) % Loop thropugh GPS to get model parameters
        ...
        gpsdates=GPS(ni).dates_years';gpsdata=GPS(ni).los';insardates=Datess;insardata=Data(ni,:);
