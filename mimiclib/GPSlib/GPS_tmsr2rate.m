...
if time_range ;  for ni = 1:length(GPS)  tmp_ind = find(GPS(ni).dateYears < time_range(1) |  GPS(ni).dateYears > time_range(2)) ;  end ;  end
...
switch(model)
    ...
    case('linear')
        ...
        for ni=1:length(GPS)
            ...
            tmp_ind = linspace(1,length(GPS(ni).dateYears),length(GPS(ni).dateYears)) ;
            
            if time_range  tmp_ind = find(GPS(ni).dateYears > time_range(1) &  GPS(ni).dateYears < time_range(2));  end
            
            if ~length(tmp_ind) ;  disp(sprintf('No GPS measurement in the given time range for station')) ;

            else
                if isfield(GPS(ni),'e')
                    tmp                 = pinv([GPS(ni).dateYears ones(size(GPS(ni).dateYears,1),1)]) * GPS(ni).e   ;
                    GPS(ni).e_rate      = tmp(1)                                                                      ;
                end
                if isfield(GPS(ni),'n')
                    tmp                 = pinv([GPS(ni).dateYears ones(size(GPS(ni).dateYears,1),1)]) * GPS(ni).n   ;
                    GPS(ni).n_rate      = tmp(1)                                                                      ;
                end
                if isfield(GPS(ni),'u')
                    tmp                 = pinv([GPS(ni).dateYears ones(size(GPS(ni).dateYears,1),1)]) * GPS(ni).u   ;
                    GPS(ni).u_rate      = tmp(1)                                                                      ;
                end
                if isfield(GPS(ni),'los')
                    tmp                 = pinv([GPS(ni).dateYears ones(size(GPS(ni).dateYears,1),1)]) * GPS(ni).los ;
    ...
    case('seasonal')
        ...
        for ni=1:length(GPS) % Loop thropugh GPS to get model parameters
            ...
            gpsdates = GPS(ni).dateYears' ;  gpsdata = GPS(ni).los' ;  insardates = Datess ;  insardata = Data(ni,:) ;
        ...
        %                        tmp=pinv([GPS(ni).dateYears ones(size(GPS(ni).dateYears,1),1)])*GPS(ni).lon;
        %                        GPS(ni).lon_rate=tmp(1);
        %                end
        %                if isfield(GPS(ni),'lat')
        %                        tmp=pinv([GPS(ni).dateYears ones(size(GPS(ni).dateYears,1),1)])*GPS(ni).lat;
