...
switch lower(origin)
    ...
    case('roi_pac')
        ...
        for ni=1:length(infiles)
            ...
            if RscInfo.FILE_LENGTH < YMIN0
                YMIN0 = RscInfo.FILE_LENGTH ;
            end
        end
        ...
        for ni=1:length(infiles)
            ...
            [~,~,satbeam,sat] = extract_ProjectName(infiles(ni).name) ;  igramtmp.sat = satbeam ;
    ...
    case('dem')
        ...
        [~,dem_data,Infos]=readfile(datafile)    ;  %[Infos]=ReadKeywordfile([datafile,'.rsc']);
