...
if isfield(dataset(1),'fulldata')
   ...
   if D_1  if NORM_1 normalization=[normalization;dataset(1).normalization'*sigphi(1)]; else tmp = [sigphi(1).*ones(Ndata(1),1)]; tmp = tmp.^2; normalization = tmp;               end; end;
   if D_2  if NORM_2 normalization=[normalization;dataset(2).normalization'*sigphi(2)]; else tmp = [sigphi(2).*ones(Ndata(2),1)]; tmp = tmp.^2; normalization=[normalization;tmp] ;end; end; 
   if D_3  if NORM_3 normalization=[normalization;dataset(3).normalization'*sigphi(3)]; else tmp = [sigphi(3).*ones(Ndata(3),1)]; tmp = tmp.^2; normalization=[normalization;tmp] ;end; end; 
   if D_4  if NORM_4 normalization=[normalization;dataset(4).normalization'*sigphi(4)]; else tmp = [sigphi(4).*ones(Ndata(4),1)]; tmp = tmp.^2; normalization=[normalization;tmp] ;end; end; 
   if D_5  if NORM_5 normalization=[normalization;dataset(5).normalization'*sigphi(5)]; else tmp = [sigphi(5).*ones(Ndata(5),1)]; tmp = tmp.^2; normalization=[normalization;tmp] ;end; end; 
   if D_6  if NORM_6 normalization=[normalization;dataset(6).normalization'*sigphi(6)]; else tmp = [sigphi(6).*ones(Ndata(6),1)]; tmp = tmp.^2; normalization=[normalization;tmp] ;end; end; 
   if D_7  if NORM_7 normalization=[normalization;dataset(7).normalization'*sigphi(7)]; else tmp = [sigphi(7).*ones(Ndata(7),1)]; tmp = tmp.^2; normalization=[normalization;tmp] ;end; end; 
   if D_8  if NORM_8 normalization=[normalization;dataset(8).normalization'*sigphi(5)]; else tmp = [sigphi(8).*ones(Ndata(8),1)]; tmp = tmp.^2; normalization=[normalization;tmp] ;end; end; 
